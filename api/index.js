...
app.get("/api/health", (_,res)=>res.json({status:"ok",app:"Bank2BiS",version:"1.3.0"}));
...
app.get("/api/item-search", async (req,res)=>{
  ...
  const url = "https://oldschool.runescape.wiki/api.php?action=query&format=json&list=search&srlimit=30&srsearch="+encodeURIComponent(q+" insource:{{Infobox Item");
  try{
    const r=await fetch(url,{headers:{"User-Agent":"Bank2BiS/1.4 (OSRS bank optimiser)"}});
    if(!r.ok) throw new Error("Wiki search failed");
    const d=await r.json();
    res.json({items:(d.query?.search||[]).map(x=>({title:x.title}))});
  }catch(e){res.status(502).json({items:[],error:"Wiki item search unavailable"})}
});
...
app.post("/api/recognize-bank", upload.single("bank"), async (req,res)=>{
  if(!req.file) return res.status(400).json({error:"No screenshot uploaded"});
  const slots = JSON.parse(req.body.slots||"[]");
  // Provider-neutral hook. A production deployment can connect a vision API by
  // implementing this block without exposing the provider key to the browser.
  if(!process.env.BANK2BIS_VISION_URL) {
    return res.json({
      status:"needs_provider",
      message:"No automatic vision provider is configured.",
      slots:slots.length,
      items:[]
    });
  }
  try{
    ...
    const r = await fetch(process.env.BANK2BIS_VISION_URL,{
      method:"POST",
      headers:{"Content-Type":"application/json","Authorization":process.env.BANK2BIS_VISION_TOKEN?`Bearer ${process.env.BANK2BIS_VISION_TOKEN}`:""},
      body
    });
    if(!r.ok) throw new Error("vision provider failed");
    return res.json(await r.json());
  }catch(e){
    return res.status(502).json({status:"error",message:"Vision provider failed; use manual review."});
  }
});
